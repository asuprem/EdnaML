...
class SaveMetadata:
    ...
    def generate_save_names_from_config(cfg: EdnaMLConfig) -> Tuple[str]:
        ...
        MODEL_SAVE_FOLDER = "%s-v%i-%s-%s"%(cfg.SAVE.MODEL_CORE_NAME, cfg.SAVE.MODEL_VERSION, cfg.SAVE.MODEL_BACKBONE, cfg.SAVE.MODEL_QUALIFIER)
        LOGGER_SAVE_NAME = "%s-v%i-%s-%s-logger.log"%(cfg.SAVE.MODEL_CORE_NAME, cfg.SAVE.MODEL_VERSION, cfg.SAVE.MODEL_BACKBONE, cfg.SAVE.MODEL_QUALIFIER)
        if cfg.SAVE.DRIVE_BACKUP:
            CHECKPOINT_DIRECTORY = os.path.join(cfg.SAVE.CHECKPOINT_DIRECTORY, MODEL_SAVE_FOLDER)
        else:
