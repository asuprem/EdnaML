...
class AzureStorage(BaseStorage):
    def build_params(self, **kwargs):
        ...
        self.local_azfilename = kwargs.get(
            "local_filename", self.blobname
        )  # "../Data/cars_datasets_models2"
        self.uploadblobname = kwargs.get(
            "uploadblobname", "default"
        )  # TODO this needs to be dealt with w.r.t. config saves...

    def read(self):
        blob_service_client_instance = BlockBlobService(
            account_name=self.storage_account, account_key=self.storage_account_key
        )
        ...
        blob_service_client_instance.get_blob_to_path(
            self.containername, self.blobname, downloadpath
        )
    ...
    def write(self, data):  # TODO should upload name be specified here as well...????
        ...
        blob_service_client_instance = BlockBlobService(
            account_name=self.storage_account, credential=self.storage_account_key
        )
        ...
        blob_client_instance_upload.upload_blob(
            data
        )  # TODO Possibly save data from somewhere. Is it a stream...need to read up on this
