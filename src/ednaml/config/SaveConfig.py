...
class SaveConfig(BaseConfig):
    ...
    def __init__(self, save_dict, defaults: ConfigDefaults):
        ...
        self.LOG_BACKUP = BackupOptionsConfig(
            save_dict.get("LOG_BACKUP", save_dict.get("BACKUP", {})), defaults
        )
        self.MODEL_BACKUP = BackupOptionsConfig(
            save_dict.get("MODEL_BACKUP", save_dict.get("BACKUP", {})), defaults
        )
        self.ARTIFACTS_BACKUP = BackupOptionsConfig(
            save_dict.get(
                "ARTIFACTS_BACKUP",
                save_dict.get("MODEL_BACKUP", save_dict.get("BACKUP", {})),
            ),
            defaults,
        )
        self.CONFIG_BACKUP = BackupOptionsConfig(
            save_dict.get("CONFIG_BACKUP", save_dict.get("BACKUP", {})), defaults
        )
        self.PLUGIN_BACKUP = BackupOptionsConfig(
            save_dict.get(
                "PLUGIN_BACKUP",
                save_dict.get("MODEL_BACKUP", save_dict.get("BACKUP", {})),
            ),
