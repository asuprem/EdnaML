...
class BaseDeploy:
    ...
    def deploy(
        ...
        continue_step: int = None,
        inference=False,
        ignore_plugins: List[str] = [],
        execute: bool = True,
        model_build: bool = None,
        **kwargs
    ):
        ...
        if continue_epoch == -1:
            self.logger.info("Starting deployment from scratch.")
            self.logger.info(
            ...
            self.logger.info(
                "Starting deployment. with `continue_epoch` %i and `continue_step` %i"
                % (continue_epoch, continue_step)
            )
            if model_build or (model_build is None and not self.model_is_built):
                self.logger.info(
                    "`model_build` is True or model may not be built. Checking."
                )
                if continue_epoch or continue_step:
                    if self.edna_context.MODEL_HAS_LOADED_WEIGHTS:
                        ...
                        self.logger.info(
                            "Model is empty and `model_build` is set. Attempting loading weights from Epoch %i Step %i"
                            % (continue_epoch, continue_step)
                        )
                        response = self.load(
                            continue_epoch,
                        ...
                        self.logger.info(
                            "Model is empty and `model_build` is set. Attempting loading plugins from Epoch %i Step %i"
                            % (continue_epoch, continue_step)
                        )
                        plugin_response = self.load(
                            continue_epoch,
                            continue_step,
                            ignore_plugins=ignore_plugins,
                            ignore_if_error=True,
                            artifact=StorageArtifactType.PLUGIN,
                        )
                        if not response:
                            raise RuntimeError(
                        ...
                        if not plugin_response:
                            self.logger.info(
                                "Could not load plugins at epoch-step %i/%i."
                                % (continue_epoch, continue_step)
                            )
                ...
                if model_build is not None and not model_build:
                    self.logger.info(
                        "Skipping model building and plugin loading due to `model_build=False`"
                    )
                elif self.model_is_built:
                    self.logger.info(
                        "Skipping model building and plugin loading because model is already built in a prior call to `deploy()`. To force, set the `model_build` flag to True in `ed.deploy`"
                    )
                else:
        ...
        self.logger.info(
            ...
            % self.storage_manager.getLocalSavePath(
                self.storage_manager.getERSKey(
                    epoch=0, step=0, artifact_type=StorageArtifactType.PLUGIN
                )
            )
        )
        ...
        if execute:
            self.logger.info(
                "Setting up plugin hooks. Plugins will fire during:  %s"
                % self.config.DEPLOYMENT.PLUGIN.HOOKS
            )
            ...
            self.logger.info(
                "Skipping execution of deployment task due to `execute = False`."
            )
    ...
    def output_setup(
        self, **kwargs
    ):  # USER IMPLEMENTS; kwargs from config.DEPLOYMENT.OUTPUT_ARGS
        self.logger.warn("No output setup is performed")

    def output_step(
        self, logits, features, secondary
    ):  # USER IMPLEMENTS, ALSO, NEED SOME STEP LOGGING...????????
    ...
    def check_epoch_save(self, epoch):  # TODO off by one errors
        ...
        if self.storage_manager.getUploadTriggerForEpoch(
            epoch, StorageArtifactType.MODEL
        ):
        ...
        if self.storage_manager.getUploadTriggerForEpoch(
            ...
            self.save(
                artifact=StorageArtifactType.PLUGIN, save_epoch=epoch, save_step=0
            )
        if self.storage_manager.getUploadTriggerForEpoch(
        ...
        if self.storage_manager.getUploadTriggerForEpoch(
            ...
            self.save(
                artifact=StorageArtifactType.METRIC, save_epoch=epoch, save_step=0
            )
        if self.storage_manager.getUploadTriggerForStep(
            epoch, StorageArtifactType.CONFIG
        ):
            self.save(
                artifact=StorageArtifactType.CONFIG, save_epoch=epoch, save_step=0
            )
    ...
    def load(
        ...
        if artifact == StorageArtifactType.MODEL:
            ...
            raise NotImplementedError(
                "CONFIG downloads should not be managed inside Deploy"
            )
        elif artifact == StorageArtifactType.LOG:
            raise NotImplementedError(
                "LOG downloads should not be managed inside Deploy"
            )
        elif artifact == StorageArtifactType.PLUGIN:
            plugin_ers_key: ERSKey = self.storage_manager.getERSKey(
                epoch=load_epoch, step=load_step, artifact_type=artifact
            )
            response = self.storage_manager.download(
            ...
            if response:
                plugin_local_savepath = self.storage_manager.getLocalSavePath(
                    ers_key=plugin_ers_key
                )
                self.model.loadPlugins(
                    plugin_local_savepath, ignore_plugins=ignore_plugins
                )
                self.logger.info(
            ...
            raise NotImplementedError(
                "METRIC downloads should not be managed inside Deploy"
            )
        else:
    ...
    def save(
        ...
        if self.storage_manager.storage_mode == "local":
            ...
            if artifact == StorageArtifactType.MODEL:
                ...
                plugin_ers_key = self.storage_manager.getERSKey(
                    epoch=save_epoch, step=save_step, artifact_type=artifact
                )
                plugin_local_path = self.storage_manager.getLocalSavePath(
                    ers_key=plugin_ers_key
                )
                ...
                metrics_ers_key = self.storage_manager.getERSKey(
                    epoch=save_epoch,
                    step=save_step,
                    artifact_type=StorageArtifactType.METRIC,
                )
