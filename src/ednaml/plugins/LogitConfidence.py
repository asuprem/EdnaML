...
class LogitConfidence(ModelPlugin):
    ...
    def post_epoch(self, model: ModelAbstract, epoch: int = 0, **kwargs):
        if (
            ...
            self.logit_confidence = (
                self.logit_confidence_logits / self.logit_confidence_count
            )
            self.activated = True
    ...
    def compute_labels(self, feature_logits):
        ...
        soft_maxes = torch.max(
            torch.nn.functional.softmax(feature_logits.cpu(), dim=1), dim=1
        )
