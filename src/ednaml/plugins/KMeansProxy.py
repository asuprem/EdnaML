...
class KMeansProxy(ModelPlugin):
    ...
    def post_forward(
        ...
        if not self.activated:
            ...
            return (
                ...
                secondary_outputs,
                kwargs,
                {"cluster_mean": labels, "distance": dist},
            )
    ...
    def post_epoch(self, model: ModelAbstract, epoch: int = 0, **kwargs):
        ...
        if self.activated and self.kdcluster is None:
            ...
            if self.dist == "euclidean":
                ...
                self.kdcluster = KDTree(
                    torch.nn.functional.normalize(self.cluster_means)
                )
            else:
            ...
            with torch.no_grad():
                self.labels = torch.argmax(
                    model.classifier(self.cluster_means.cuda()), dim=1
                ).cpu()
    ...
    def compute_labels(self, batch):
        ...
        dist, idx = self.kdcluster.query(
            self._preprocess(q_batch), k=1, return_distance=True
        )  # .squeeze()
        # TODO convert idx to the actual cluster means to the actual cluster labels...
        return torch.from_numpy(dist).squeeze(1), torch.stack(
            [self.labels[item[0]] for item in idx]
        )
