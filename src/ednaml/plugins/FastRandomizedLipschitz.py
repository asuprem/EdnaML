...
class FastRandomizedLipschitz(ModelPlugin):
    ...
    def build_plugin(self, **kwargs):
        ...
        self.perturbation_neighbors = kwargs.get(
            "perturbation_neighbors", self.neighbors
        )
        ...
        if output_file is None:
            ...
            output_file = "-".join(
                ["FRL", "features", uuid.uuid1().urn.split("-")[0].split(":")[-1]]
            )
    ...
    def post_forward(
        ...
        if not self.activated:
            ...
            lscore, smoothlscore, proxy_labels = self.compute_lscores(
                features, feature_logits, model
            )
            ...
            return (
                ...
                {
                    "l_score": lscore,
                    "smooth_l_score": smoothlscore,
                    "l_threshold": [self.lipschitz_threshold] * features.shape[0],
                    "smooth_l_threshold": [self.smooth_lipschitz_threshold]
                    * features.shape[0],
    ...
    def compute_lscores(self, features, feature_logits, model):
        perturbed = self.generate_perturbation(
            self.epsilon, self.dimensions, self.perturbation_neighbors
        ).T.cuda()
        if self.epsilon != self.l_epsilon:
            l_perturbed = self.generate_perturbation(
                self.l_epsilon, self.dimensions, self.perturbation_neighbors
            ).T.cuda()
        ...
        for j, x in enumerate(features):
            ...
            with torch.no_grad():
                ...
                if self.epsilon != self.l_epsilon:
                    ...
                    logit_lscore = torch.sqrt(
                        ((l_perturbed_logits - raw_logits) ** 2).sum(1)
                    )
                else:
                    logit_lscore = torch.sqrt(
                        ((perturbed_logits - raw_logits) ** 2).sum(1)
                    )
                l_scores = (
                    logit_lscore[feature_lscore > 0]
                    / feature_lscore[feature_lscore > 0]
                )
                lscore[j] = torch.max(l_scores).cpu().item()
                smoothlogit_lscore = (
                    perturbed_logits[:, torch.argmax(raw_logits)]
                    - raw_logits[:, torch.argmax(raw_logits)]
                )
                smooth_lscores = (
                    smoothlogit_lscore[feature_lscore > 0]
                    / feature_lscore[feature_lscore > 0]
                )
                smooth_lscore[j] = torch.max(smooth_lscores).cpu().item()

        _, idx = self.kdcluster.query(
            self._preprocess(features.cpu()), k=1, return_distance=True
        )  # .squeeze()
    ...
    def post_epoch(self, model: ModelAbstract, epoch: int = 0, **kwargs):
        if (
            ...
            if self.proxy_stage:  # Check if we are ready to move to lipschitz stage.
                ...
                self.lipschitz_stage = (
                    self.epoch_count >= self.proxy_epochs
                )  # better way to deal with this whole situation, i.e. once activated, never changes...!
                ...
                if self.proxy_stage:
                    ...
                    self.high_density_thresholds = torch.tensor(
                        self.high_density_thresholds
                    )
                ...
                self._logger.info(
                    "RandomizedLipschitz has completed Lipschitz stage. Computing L values."
                )
                ...
                with torch.no_grad():
                    for idx in range(len(self.cluster_means)):
                        raw_logits = self._classifier(
                            self.cluster_means[idx].unsqueeze(0).cuda()
                        ).cpu()
                        lipschitz_logits = self._classifier(
                            torch.stack(
                                [item[0] for item in self._closest_features[idx]]
                            ).cuda()
                        ).cpu()

                        # use euclidean here, only!!!!!!
                        feature_lscore = torch.sqrt(
                            (
                                (
                                    torch.stack(
                                        [
                                            item[0]
                                            for item in self._closest_features[idx]
                                        ]
                                    )
                                    - self.cluster_means[idx].unsqueeze(0)
                                )
                                ** 2
                            ).sum(1)
                        )
                        logit_lscore = torch.sqrt(
                            ((lipschitz_logits - raw_logits) ** 2).sum(1)
                        )

                        smoothlogit_lscore = (
                            lipschitz_logits[:, torch.argmax(raw_logits)]
                            - raw_logits[:, torch.argmax(raw_logits)]
                        )
                ...
                self.epsilon = 1.0 / self.smooth_lipschitz_threshold
                self.l_epsilon = 1.0 / self.lipschitz_threshold
        ...
        if (
            self.lipschitz_stage
            and self._closest_features is None
            ...
            self._closest_features = [
                SortedKeyList(key=lambda x: x[1])
                for _ in range(len(self.cluster_means))
            ]  # x <- (feature, distance)
    ...
    def search_minibatch_kmeans_clusters(
        self,
        batch: torch.Tensor,
    ):
        ...
        for _, x in enumerate(local_batch):
            ...
            # basically -->
            [
                feature_list.add((x, resp[idx].item()))
                for idx, feature_list in enumerate(self._closest_features)
            ]

        # prune each to be length of self.neighbors...
        self._closest_features = [
            SortedKeyList(feature_list[: self.neighbors], key=feature_list.key)
            for idx, feature_list in enumerate(self._closest_features)
        ]
    ...
    def generate_perturbation(self, epsilon, n_dims, n_samples):
        Y = np.random.multivariate_normal(
            mean=[0], cov=np.eye(1, 1), size=(n_dims, n_samples)
        )
        Y = np.squeeze(Y, -1)
