    ...
    def __init__(self, logger, dataset, mode, transform=None, **kwargs):
        ...
        if os.path.exists(self.base_cachepath + ".h5"):
            if self.cache_replace:
                ...
                shutil.rmtree(
                    self.cachepath
                )  # TODO this will delete the entire tree!!!!!!
            else:
        ...
        if self.shardcache:
            ...
            if not self.shards_exist:
                ...
                self.shardsaveindex = len(
                    glob(os.path.join(self.shardpath, "*.pt"))
                )  # TODO Bug fix if files do not have consistent numbering
    ...
    def sharded_refresh_mask_ids(self, sharded_dataset: TensorDataset):
        ...
        if self.token_masking or self.word_masking:
            ...
            return TensorDataset(
                all_input_ids,
                all_attention_mask,
                torch.stack([shard[3] for shard in sharded_dataset]),
                all_masklm,
                torch.stack([shard[4] for shard in sharded_dataset]),
                torch.stack([shard[5] for shard in sharded_dataset]),
