# Inherited from fnc-extended-labeling-crawler.py...
...
class FNCCrawler(Crawler):
    ...
    def __init__(
        ...
        azcontainer="edna-covid-keywords",
        azfile="mask.json.gz",
        test_file = None,
        ...
        if az_jsonfile is not None:
            self.metadata["test"]["crawl"] = IterableFile(
                az_jsonfile,
                line_callback=self.constructor,
            )
        if train_az_jsonfile is not None:
    ...
    def constructor(self, row):
        jl = json.loads(row.strip())
        return (
            jl["full_text"],
            jl["timestamp_ms"],
            int(jl["retweet_count"]),
            int(jl["favorite_count"]),
            0   # label
        )
